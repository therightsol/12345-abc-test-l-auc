...
abstract class QueryFilter
{
    ...
    protected function order()
    {
        ...
        $collection = $this->belongsToThrough + [class_basename($this->model) =>$this->column] + $this->belongsTo;

        $table = ucfirst($explode[0]);
        $column = $explode[1];

        if (!array_key_exists($table, $collection))  return;
        if (!in_array($column, $collection[$table])) return;



        $model = $this->modelPath . $table;
        $instance = new $model;
