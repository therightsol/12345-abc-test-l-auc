                                    ...
                                    <table class="table table-striped table-hover dataTable">
                                        ...
                                        @foreach($biddings as $bid)
                                            <tr @if($bid->auction->winner_user_id == \Auth::user()->id) style="    border: 1px solid green;" title="Won" @endif>
                                                ...
                                                <td><a href="{{ url('view-auction/'.$bid->auction_id) }}" target="_blank">{{ $bid->title }}</a></td>
                                                <td width="150">

                                                    <a target="_blank" href="{{ url('view-auction/'.$bid->auction_id) }}"  class="btn delete-row btn-icon-toggle"
                                                            >
                                                        <i class="fa fa-eye"></i>
                                                    </a>
